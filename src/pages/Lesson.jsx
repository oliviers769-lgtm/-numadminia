...
const MISTRAL_KEY = process.env.REACT_APP_MISTRAL_KEY || '1ppX4590CDvOxMNzmx0FzadRBOiTvytH';
...
async function getMistralFeedback({ lesson, chosenText, lang }) {
  ...
  const prompt = `Tu es un tuteur bienveillant pour une app d'aide administrative française. ${langInstructions[lang] || langInstructions.fr}

L'utilisateur a répondu "${chosenText}" à la question "${lesson.question}", mais la bonne réponse est "${correctOpt?.text}".

Écris 2-3 phrases maximum pour expliquer son erreur et la bonne réponse. Vocabulaire très simple, ton encourageant. Réponds uniquement avec l'explication, sans guillemets, sans astérisques, sans méta-commentaires.`;
